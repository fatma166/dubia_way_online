...
use App\Repositories\Admin\Vendor\VendorEmployeeRepository;
...
class VendorEmployeeController extends BaseController
{
    ...
    public function __construct(VendorEmployeeRepository $repository)
    {
        parent::__construct($repository);
        $this->view = 'vendor-views.vendor_employee.';

    }
    public function index(Request $request, $with = [], $withCount = [], $filter = '', $paginate = 10, $whereHas = [])
    {
       $employees= parent::index($request, ['role'], [], '', 10, []);
    ...
    public function store(Request $request)
    {
       ...
       //return redirect(route('vendor.employee.index'));
